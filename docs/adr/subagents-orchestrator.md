# ADR：pi-agent-config 不为 vendored subagents 默认启用 always-on orchestrator
...
本 ADR 只约束 `pi-agent-config` 仓库内 vendored 的 `extensions/subagents` 及与它配套的 `skills/orchestrator/SKILL.md`，不替上游 `HazAT/pi-interactive-subagents` 或其他安装者作产品决策。

本地 fork 提供异步子代理工具、角色定义、并行派遣、mux 生命周期管理和 steer 自动回传，并维护适合本项目的模型、agent prompts 与 `/plan` 工作流。可用角色包括 `scout`、`researcher`、`planner`、`worker`、`reviewer` 等。

曾考虑在本项目中增加一个自动、常驻的 orchestrator，通过 system prompt 或 `promptGuidelines` 指导主代理在每个任务中主动选择和派遣子代理。
...
三路调研结论一致：本地架构已经合理，不应在没有实际问题和评测证据时增加默认 orchestrator。外部项目只作为证据来源，不是本 ADR 的决策对象。

## 决策

维持 `pi-agent-config` 当前的分层设计，不为本地 vendored `subagents` 默认启用 always-on orchestrator：
...
`skills/orchestrator/SKILL.md` 保持为按需能力，不设为 always-on。它必须以本地 agent frontmatter 和实际注册工具为准；当前已修正 worker spawning、工具列表、并发上限和上下文继承等旧偏差。

本地 `/plan` 使用同一个 `plan.md` 保存设计和 `## Implementation Tasks`，不依赖未安装的 `todo` 工具、`write-todos` 或 `commit` skill。该改造属于按需工作流内部实现，不改变“不启用常驻 orchestrator”的决定。

## 复评结果

本 ADR 在本地一致性改造完成后重新评估，结论维持不变：

- `orchestrator` skill 已与本地 `scout`、`researcher`、`worker` 权限对齐；
- planner → `plan.md` → worker → reviewer 的 prompt contract 已改为自包含；
- `extensions/subagents/test/test.ts` 新增了可选 workflow resource 的回归检查；
- 父会话测试结果为 136/136 通过；
- YouTube 内容验证任务作为边界案例，显示一次性外部研究更适合直接使用 researcher/OpenCLI，而不是强制进入 `/plan` 开发流水线。

这次复评没有发现需要修改 `pi-extension/subagents/index.ts` 的运行时缺陷。`visual-tester` 的依赖适配是独立交付问题，不属于是否启用 always-on orchestrator 的决策范围。
...
- `.pi/plans/2026-07-17-subagents-consistency/plan.md`
