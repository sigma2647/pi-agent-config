...
│   ├── providers/       ← one file per vendor (openai-compatible, deepgram)
│   ├── local/           ← catalog + model download/delete + sherpa-onnx provider
...
**Key handling is toggle-or-hold, and no app shortcut is registered.** `keybindMode: "toggle"` (default) is press-to-start / press-to-stop; `"hold"` records while the hotkey is held and finishes on key release, which needs a terminal that reports key releases (the Kitty keyboard protocol with the report-event-types flag). pi-tui asks for flags=7 on startup, so ghostty/kitty/wezterm do report releases. tmux swallows them: with `extended-keys on csi-u` it forwards modified keys as CSI-u, but it never asks the outer terminal for event types and has no kitty-protocol support (absent from tmux through 3.8). herdr forwards them: its client pushes flags=7 to the outer terminal, `pane::kitty_keyboard::KittyKeyboardTracker` records what each pane pushed, and `server::pane_input::apply_client_terminal_input_events` re-encodes client input per pane with ghostty's key encoder — herdr's own troubleshooting doc confirms release bytes reach pane apps (duplicated Enter/Backspace, issue #1116). A pi session under tmux therefore degrades to toggle on the first second press, with a one-time notice; under herdr it should hold, which `pi-dictation keytest` confirms in one press. `pi-dictation keytest` shows what a terminal actually sends. `pi.registerShortcut` is deliberately NOT used for ctrl+r: it makes pi print an "Extension shortcut conflict" note on every start (ctrl+r is also `app.session.rename`). The key is consumed earlier instead, in `ctx.ui.onTerminalInput`, which also beats the built-in binding; the wrapping editor component is the fallback path when that API is absent. Auto-repeat events are swallowed so holding the key cannot stop the recording. The wrapper must forward host duck-typed members (`onCtrlD`, `onEscape`, `onPasteImage`, `actionHandlers`, `onExtensionShortcut`) — pi copies its default-editor handlers onto a custom editor only when those exist, so dropping them breaks Ctrl+D, Escape and every other extension's shortcut. `ui.ts` has no runtime pi imports — the host injects `EditorHelpers` (`setEditorHelpers` is the test seam), which keeps the pure render helpers testable under plain Node.
