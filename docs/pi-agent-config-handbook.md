...
**Local patch — pane width floor (`cmux.ts`, diverges from upstream 3.7.2):** upstream splits every subagent pane off the parent pi pane (`herdr pane split` / `tmux split-window -h`), so a 16-way fan-out halves the parent until panes are 1 column wide. pi dies in a 1-column pane as soon as a double-width glyph (emoji) is rendered: regular TUI mode throws `Rendered line N exceeds terminal width`, fullscreen mode recurses forever in `wordWrapLine`. `createSurface` now measures the parent pane (`herdr pane layout --pane`, `tmux display -p '#{pane_width}'`) and, when it is narrower than `MIN_SUBAGENT_PANE_WIDTH * 2` (80 columns), opens a full-width `herdr tab create` / `tmux new-window` instead of splitting. An unmeasurable width splits as before. Keep this patch when re-vendoring the package.
