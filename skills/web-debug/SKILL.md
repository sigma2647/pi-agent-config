---
name: web-debug
description: "Debug or verify frontend behavior by driving a live page (DOM, storage, network, console) with the browser_probe tool instead of reading source or asking the user to paste from devtools. Use when the user reports: broken login or auth flow, failed/401/403/CORS requests, JWT or session weirdness, form not submitting, button does nothing, blank screen, hydration mismatch, stale data, 'works locally / fails in prod', or asks you to verify a frontend change end-to-end."
---
...
the browser), still open the page first. You will learn more in three tool calls
than three rounds of source-reading.

## Tool mapping (this machine)

The playbooks below were written for a Playwright `browser_*` tool set. Here the
same jobs are done by the native `browser_probe` tool (argv style, no binary
name). The code blocks keep the old names for readability — translate each line
with this table before running it.

| Playbook step | Do this instead |
|---|---|
| `browser_goto url=X` | `browser_probe navigate X` (no page yet: `open X`) |
| `browser_eval expression=E` | `browser_probe eval 'E'`, or `browser_probe exec` with `await eval('E')` |
| `browser_console` | `browser_probe console` (`--errors`, `--clear`) |
| `browser_network urlFilter=F verbose=true` | `browser_probe network requests F`, then `browser_probe network request <id>` for headers/body |
| `browser_fill selector=S value=V` | `browser_probe exec` → `fill([[ref, V]])` or `input(ref, V)`; get `ref` from `browser_probe state -i` |
| `browser_click selector=S` | `browser_probe click <ref>` (ref from `state -i`); use `browser_probe find "<text>"` to locate it |
| `browser_screenshot` | `browser_probe screenshot` |
| `browser_close` | not needed — state persists in `~/.browser-probe/`; `browser_probe disconnect` only detaches |

`browser_probe <cmd> --help` is authoritative for flags. The persistent profile
behaves like the Playwright one: cookies, localStorage and IndexedDB survive
across calls, turns, and pi restarts.

## Core loop

Every playbook below is a variation on this:

1. `browser_probe navigate <url>` to the relevant URL.
2. Drive whatever action reproduces the bug (`browser_probe exec` with `fill` / `click`).
3. Drain observations: `browser_probe console`, `browser_probe network requests <filter>`.
4. `browser_probe eval '<expr>'` to read runtime state that isn't visible from
   console/network.
5. Form a hypothesis. Make a code change. Re-run the loop to verify.

State (cookies, localStorage, IndexedDB) is persistent across `browser_probe`
calls, across turns, and across pi restarts — a session you opened earlier
is still open now. That's a feature: don't disconnect between steps.
...
- **DOM nodes don't JSON-serialize.** `browser_probe eval` returns a
  `{ __nonSerializable, description, hint }` sentinel instead. Return primitive
  properties: `.outerHTML`, `.textContent`, `.value`, `.checked`. Never return
  the node itself or `document.body`.
...
- **`console` / `network` capture lazily.** The first `browser_probe console`
  call starts buffering, so entries logged before it are gone — read once,
  drive the page, read again. `--clear` is opt-in; the buffer is not drained
  by default.
- **Top-level `return` and multi-statement bodies aren't expressions.** Wrap
  them in `(() => { ... })()` when passing to `browser_probe eval`.
